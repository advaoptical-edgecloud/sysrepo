...
sr_diff_find_cid(struct lyd_node *diff, sr_cid_t *cid, int *meta_own)
{
    ...
    struct lyd_meta *meta, *cid_meta = NULL;
    ...
    for (parent = diff; parent; parent = lyd_parent(parent)) {
        LY_LIST_FOR(parent->meta, meta) {
            if (!cid_meta && !strcmp(meta->name, "cid") && !strcmp(meta->annotation->module->name, SR_YANG_MOD)) {
                cid_meta = meta;
                break;
            }
        }
...
sr_diff_check_cid(struct lyd_node *diff_node, sr_cid_t cur_cid, int cur_meta_own, sr_cid_t cid, int keep_cur_child)
{
...
sr_lyd_diff_merge_cb(const struct lyd_node *src_node, struct lyd_node *trg_node, void *cid_p)
{
    ...
    /* learn current CID */
    sr_diff_find_cid(trg_node, &cur_cid, &meta_own);

    /* check CID of the new node */
    if ((err_info = sr_diff_check_cid(trg_node, cur_cid, meta_own, *((sr_cid_t *)cid_p), 1))) {
        goto error;
    }
